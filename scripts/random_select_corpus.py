### Randomly select texts to build language models

import io, os, argparse, random
import pandas as pd

parser = argparse.ArgumentParser()
parser.add_argument('--path', type = str, help = 'e.g., data/fongbe')
parser.add_argument('--corpus', type = str, help = 'e.g., data/fongbe/local/corpus.txt')
args = parser.parse_args()

#total_num_of_words = 41386
proportion = 1.3
train_data_num_of_words = 0
f = ''
if 'hupa' not in args.path:
	f = open(args.path + 'random/train1/corpus', encoding = 'latin-1')
else:
	f = open(args.path + 'top_tier/random/train1/corpus', encoding = 'latin-1')

#with open(args.path + 'random/train1/corpus', encoding = 'latin-1') as f:
for line in f:
	toks = line.split()
...
for n in range(1, 4): ### how many texts for language model to construct
	...
	i = 0
	while select_num_of_words < total_num_of_words + 5:
		transcript = transcripts[i]
		select_num_of_words += len(transcript.split())
		select_texts.append(transcript)
		i += 1

		if select_num_of_words >= total_num_of_words + 5:
			break
